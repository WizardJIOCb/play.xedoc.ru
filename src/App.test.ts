...
import { getBootstrap, getGlobalTop, getGlobalTopSection, getListeningStats, getPublicShare } from './lib/api'
...
vi.mock('./lib/api', async (importOriginal) => {
  ...
  return {
    ...
    getGlobalTopSection: vi.fn().mockResolvedValue({
      kind: 'chart', id: 'chart', title: 'Мировой чарт', total: 1, offset: 0, limit: 20, hasMore: false,
      tracks: [{ id: 'world-one', title: 'World One', artists: ['Global Artist'], durationMs: 180_000, streamUrl: '/api/public-search/tracks/world-one/stream?ticket=signed-ticket' }],
      releases: [],
    }),
...
describe('favorite collection filtering', () => {
  ...
  it('opens a global top rubric from its direct link', async () => {
    ...
    vi.mocked(getGlobalTopSection).mockClear()

    render(createElement(PlayerProvider, null, createElement(App)))

    expect(await screen.findByRole('heading', { name: 'Мировой чарт' })).toBeInTheDocument()
    expect(await screen.findByText('World One')).toBeInTheDocument()
    expect(getGlobalTop).toHaveBeenCalled()
    expect(getGlobalTopSection).toHaveBeenCalledWith('chart', undefined, 0, 20)
