  ...
  return (
    <section className="global-top-page">
      ...
      {selectedGenre && <section className="global-top-section global-top-genres">
        <header><div><span className="eyebrow">РЕЙТИНГИ ПО ЗВУЧАНИЮ</span><h2><button className="global-top-heading-button" type="button" title={`Открыть весь рейтинг «${selectedGenre.title}»`} onClick={() => openDetail({ kind: 'genre', id: selectedGenre.id, title: selectedGenre.title, description: selectedGenre.sourceTitle ? `По порядку в подборке «${selectedGenre.sourceTitle}»` : undefined })}>Жанровые рейтинги<ChevronRight size={22} /></button></h2><p>От пост-рока, шугейза, эмбиента и lo-fi до хардкора, металкора и знакомой классики жанров.</p></div><Disc3 size={22} /></header>
