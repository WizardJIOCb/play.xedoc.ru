...
export function TrackRow({ track, context, index, compact = false }: { track: Track; context: Track[]; index?: number; compact?: boolean }) {
  ...
  return (
    <div className={`track-row ${active ? 'track-row--active' : ''} ${compact ? 'track-row--compact' : ''}`}>
      ...
      <button className={`icon-button track-row__like ${liked ? 'is-liked' : ''}`} type="button" aria-label={liked ? 'Убрать лайк' : 'Поставить лайк'} disabled={liking} onClick={() => void onLike()}>
        <Heart size={17} fill={liked ? 'currentColor' : 'none'} />
      </button>
      <span className="track-row__duration">{formatDuration(track.durationMs)}</span>
      <button className="icon-button track-row__next" type="button" aria-label="Добавить следующим" onClick={() => player.addNext(track)}>
        <ListPlus size={18} />
      </button>
    </div>
  )
