...
for command_name in curl flock git install mv npm python3 readlink systemctl tar; do
...
if [[ ! -f "$release_dir/.release-ready" ]]; then
    ...
    python3 -m venv "$staging_dir/backend/.venv"
    "$staging_dir/backend/.venv/bin/python" -m pip install \
